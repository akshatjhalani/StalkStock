...
def run(event, context):
    ...
    try:
        with psycopg2.connect(
            ...
            current_hour_datetime = utils.get_current_hour_datetime()

            cur = conn.cursor()
            cur.execute(f"SELECT * FROM hourly_fetcher_meta f WHERE f.last_fetched < '{current_hour_datetime}' ORDER BY f.last_fetched ASC LIMIT 7;")
            data = cur.fetchall()
            if len(data) == 0:
            ...
            symbol = data[idx][0]
            last_fetched = data[idx][1].astimezone(sgtz)
            end_datetime = utils.to_next_month(last_fetched)

            if current_hour_datetime < end_datetime:
                end_datetime = current_hour_datetime
            logger.info(f"Fetching data for {symbol} from {last_fetched} to {end_datetime}")

            data = yf.download(
                tickers=symbol,
                start=last_fetched.strftime('%Y-%m-%d'),
            ...
            with table.batch_writer() as batch:
                for idx, row in data.iterrows():
                    ...
                    if isnan(row['Open']) or isnan(row['Close']) or isnan(row['High']) or isnan(row['Low']) or \
                        ...
                        logger.warning(f"Skipping {fetched_datetime} for {symbol} since it contains NaN or Inf")
                        continue
                    item = {
                        'symbol': symbol,
                        'date': fetched_datetime.isoformat(),
                    ...
                    if fetched_datetime > end_datetime:
                        logger.warning(f"{item} is too new, skipping")
                        continue
                    cnt += 1
                    last_fetched = fetched_datetime

                    batch.put_item(Item=item)

            logger.info(f"{cnt} items inserted")

            cur.execute(f"UPDATE hourly_fetcher_meta SET last_fetched = '{last_fetched}' WHERE symbol = '{symbol}';")
...
def init_db(event, context):
    ...
    with psycopg2.connect(
        ...
        drop_table_query = "DROP TABLE IF EXISTS hourly_fetcher_meta;"
        cur.execute(drop_table_query)

        create_table_query = '''CREATE TABLE hourly_fetcher_meta (
...
def add_stock(event, context):
    ...
    try:
        symbol = event['symbol']
        last_fetched = datetime.datetime.fromisoformat(event['last_fetched'])
    except ValueError:
    ...
    with psycopg2.connect(
        ...
        insert_query = '''INSERT INTO hourly_fetcher_meta (symbol, last_fetched)
            VALUES (%s, %s)
...
def status(event, context):
    ...
    try:
        with psycopg2.connect(
            ...
            cur = conn.cursor()
            cur.execute(
                f"SELECT * FROM hourly_fetcher_meta f ORDER BY f.last_fetched ASC;")
            data = cur.fetchall()
